...
function iconCards() {
  return {
    ...
    blocks: {
      ...
      ...iconCard("icon-card-1", "\u{1F6E1}\uFE0F", "Enterprise Security", "Bank-level encryption protects your data with industry-leading standards."),
      ...iconCard("icon-card-2", "\u26A1", "Lightning Fast", "Optimized performance for the best user experience."),
      ...iconCard("icon-card-3", "\u{1F527}", "Easy Integration", "Simple APIs and SDKs for seamless integration."),
      ...iconCard("icon-card-4", "\u{1F4CA}", "Analytics Built-in", "Track and measure everything out of the box.")
    }
  };
}
function iconCard(id, icon, title, description) {
  return {
    [id]: {
      id,
      type: "container",
...
function comparisonRow(id, label, values, isHeader) {
  const blocks = {
    [id]: {
      id,
      type: "container",
...
var iconCardsSection = iconCards();
var faq = faqSection();
var comparison = comparisonTable();
...
export { Breakpoints, cloneDocument, createDefaultDocument, exportToHtml, instantiateSectionTemplate, mergeStyle, resolveStyle, sectionTemplates, styleToCss, validateDocument };
